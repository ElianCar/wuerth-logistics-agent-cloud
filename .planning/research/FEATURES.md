...
| Caveats and limitations slide or section | Wuerth local data has missing KPI columns and join caveats; hiding this would make the export misleading. | Medium | `reporting_result["caveats"]`, `semantic_layer/databricks/wuerth_semantic_layer.yaml`, `evaluation/wuerth_local/golden_questions.yaml` | Deck includes caveats such as missing revenue, packing cost, shipment date, plant, shipping point, unit gaps, 50-row chart cap, and many-to-many join risks when present in the reporting result. |
...
- Wuerth semantic limitations: `semantic_layer/databricks/wuerth_semantic_layer.yaml`
